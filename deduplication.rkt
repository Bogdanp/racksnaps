...
(define (dedupe-snapshot snapshot-path store-path)
  (define-values (hits misses)
    (for/fold ([hits 0]
               [misses 0])
              ([path (in-list (find-pkg-archives snapshot-path))] #:unless (link-exists? path))
      ...
      (define hit? (file-exists? target-path))
      (if hit?
          (delete-file path)
      ...
      (make-checksum path)
      (if hit?
          (values (add1 hits) misses)
          (values hits (add1 misses)))))
  (log-deduper-info "hit rate ~a/~a" hits (+ hits misses)))
